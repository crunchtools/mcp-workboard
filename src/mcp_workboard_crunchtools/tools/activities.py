...
from .formatting import add_action_item_details
...
def _format_activity(ai: dict[str, Any]) -> dict[str, Any]:
    ...
    add_action_item_details(formatted, ai)
    return formatted
...
async def list_activities(
    ...
    if ai_priority is not None:
        params["ai_priority"] = ai_priority
    if ai_effort is not None:
        params["ai_effort"] = ai_effort
...
async def create_activity(
    ...
    body = response.get("data", {})
    raw = body.get("activity", body)
    return {"activity": _format_activity(raw) if isinstance(raw, dict) else raw}
